...
export async function POST(request: Request) {
  try {
    ...
    if (isTalentProduct) {
      ...
      // Never trust customer identity from the browser for Talent Pro.
      customerId = user.id
    ...
    const displayAmount = isLocalTalentPayment ? plan.baridimobAmount : plan.amount
    const displayCurrency = isLocalTalentPayment ? plan.baridimobCurrency : 'USD'

    const rawMetadata = body.metadata && typeof body.metadata === 'object' ? body.metadata : {}
    const metadata = {
      ...rawMetadata,
      ...(isTalentProduct ? { talent_id: customerId } : {}),
      ...(isLocalTalentPayment
        ? {
            local_amount: plan.baridimobAmount,
            local_currency: plan.baridimobCurrency,
          }
        : {}),
    }
    ...
    const insertResponse = await fetch(`${SUPABASE_URL}/rest/v1/payment_orders`, {
      ...
      body: JSON.stringify({
        ...
        // amount_usd remains the canonical RedotPay price. Local BaridiMob
        // pricing is stored explicitly in metadata and returned as DZD.
