...
export function TalentProUpgrade({ onPaymentSubmitted }: Props) {
  ...
  async function submitProof() {
    ...
    if (!transactionHash.trim() && !receipt) {
      setError('Please enter the transaction ID/reference from your payment provider or upload a payment receipt.')
      return
    }
      ...
      {!details ? (
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-[#071426]/70 p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-500">Payment order reference</p>
              <p className="mt-1 break-all font-mono text-sm text-[#d8b56b]">{details.reference}</p>
              <p className="mt-1 text-xs text-slate-500">Internal order ID used by RemoteStart-DZ to match your payment and approval.</p>
            </div>
          ...
          {details.paymentMethod === 'redotpay' ? (
            <div className="mt-5 space-y-3 text-sm text-slate-300">
              ...
              <p className="text-xs leading-5 text-slate-500">After paying, enter the transaction ID/reference generated by RedotPay below. Do not enter the RemoteStart-DZ payment order reference above.</p>
            </div>
          ) : (
          ...
          <div className="mt-5 grid gap-3">
            <input
              ...
              placeholder="Transaction ID / reference from RedotPay or BaridiMob"
            ...
            <label className="rounded-xl border border-dashed border-white/15 bg-white/[.025] px-4 py-3 text-sm text-slate-400">
              <span className="block text-xs text-slate-500">Payment receipt (optional if transaction ID is provided)</span>
